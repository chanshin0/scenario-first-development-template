...
이 시스템은 그 니즈를 그대로 반영한다. 대부분의 사람에게는 구체적인 명세를 쓰는 것보다, 구체적인 사용 시나리오 하나를 떠올리는 편이 훨씬 쉽다.

**구체적으로 어떻게 쓰고 싶은지, 시나리오 하나만 던지세요.** 구조화된 파이프라인이 그 시나리오를 받아 실제로 만들어 냅니다. 안에서 무슨 일이 벌어지는지는 **몰라도 됩니다.** 같은 시나리오를 직접 해봤을 때 결과물이 의도한 대로 동작하는지, 그것만 확인하세요.
