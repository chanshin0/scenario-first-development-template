...
개발의 장벽이 무너졌고, 이제 누구나 바이브 코딩을 할 수 있는 시대가 됐다. 그렇다면 **보통 사람**(*컴퓨팅 사고를 배운 적 없는*)에게 "개발"이란 뭘까? 거창한 정의는 필요 없다. **"내가 쓰고 싶은 방식대로 동작하게 해줘."** 그게 그들이 바라는 개발이다.

이 시스템은 그 니즈를 그대로 반영한다. 다만 머릿속의 "생각한 대로"를 그대로 가져다 쓸 수는 없다. 거기엔 취향, 추측, 기능 아이디어, 아직 덜 익은 기대가 뒤섞여 있기 때문이다. SFD 는 그중 나중에 다시 말할 수 있고, 테스트할 수 있고, 직접 알아볼 수 있는 부분만 붙잡는다: **"이렇게 쓰고 싶다."** 이 사용 시나리오가 Source of Truth 다.

**쓰고 싶은 방식을 던지세요.** 구조화된 파이프라인이 그 시나리오를 받아 실제로 만들어 냅니다. 안에서 무슨 일이 벌어지는지는 **몰라도 됩니다.** 같은 시나리오를 직접 해봤을 때 결과물이 의도한 대로 동작하는지, 그것만 확인하세요.
...
이 레포는 **GitHub template** 입니다. "Use this template" 으로 clone 하면 6 스킬 + 메타 에이전트가 이미 들어있습니다 — 외부 install·마켓플레이스·trust dialog 없음.
