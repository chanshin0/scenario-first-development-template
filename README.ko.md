...
> ### 당신이 쓰고 싶은 방식에서 시작합니다.

개발의 장벽이 무너졌고, 모두가 바이브 코딩을 할 수 있는 시대가 되었다. 그렇다면 **보통 사람**(*컴퓨팅 사고를 배운 적 없는*)에게 "개발"이란 무엇일까? 지루하고 현학적인 정의는 필요 없다. **"내가 쓰고 싶은 방식대로 동작하게 해줘"** — 그게 그들이 바라는 개발이다.

이 시스템은 그 니즈를 그대로 반영한다. 머릿속의 "생각한 대로"는 그대로 쓰기엔 너무 넓다. 취향, 추측, 기능 아이디어, 덜 익은 기대가 뒤섞여 있을 수 있다. SFD 는 그중 나중에 다시 말하고, 테스트하고, 알아볼 수 있는 부분만 붙잡는다: **"이렇게 쓰고 싶다."** 이 사용 시나리오가 Source of Truth 다.

**쓰고 싶은 방식을 던지세요.** 구조화된 파이프라인이 그 시나리오를 받아 실제로 만들어 냅니다. 안에서 무슨 일이 벌어지는지는 **전혀 몰라도 됩니다.** 같은 시나리오를 직접 해봤을 때 결과물이 의도한 대로 동작하는지, 그것만 확인하세요.
...
이게 전부예요. `throw`(1) → `expand`(2) → `spec`(3) 까지 그냥 흘러가면 됩니다.
**E2E 프레임워크는 처음 `/scenario-first-goal`(4) 돌릴 때 한 번만 물어봐요** — 시작 시점에 정할 필요 없습니다.

> clone 직후엔 SFD 자신의 흔적(미치환 placeholder, 이 README)이 살짝 묻어있어요. 거슬리면 `/scenario-first-init` 한 번으로 치웁니다 — 선택이고 멱등. (자세히는 아래 [clone 정리](#clone-정리) 참고.)

---

> **여기까지가 당신이 알아야 할 전부예요.** 아래부터는 직접 만들거나, 안에서 무슨 일이 벌어지는지 궁금한 사람을 위한 내부 이야기입니다.
...
코드는 시간이 지나며 다시 쓰이고, spec 은 deprecate 된다. 하지만 *"이렇게 쓰고 싶다"* 는 마음은 비교적 안 변한다. 그래서 SoT(Source of Truth)를 그 **사용 시나리오**에 두고, `scenarios/` 로 외화(externalize)한다.
...
자동 게이트를 다 통과해도 *직접 안 쓰면* 가짜 정답이다 — "검증된 듯 쓸모없는 결과물"이 쌓이는 **주기적 자동 MVP 함정**.
...
**맞는 경우** — 본인이 직접 빌드·사용·평가하는 single-developer mode. "이렇게 쓰고 싶다"가 비교적 안정적인 도구·앱. 이미 핵심 사용 시나리오 하나가 있고 얇은 MVP 를 빠르게 검증하고 싶을 때. cycle 을 반복하며 backbone 슬라이스를 채워가는 진화적 개발.
