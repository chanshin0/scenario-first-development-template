...
AI 가 코드를 싸게 찍어내는 시대에, 진짜 어려운 건 *코드를 짜는 것*이 아니라 *"맞는 걸 만들었나"* 다.

코드는 다시 쓰이고 spec 은 deprecate 되지만, "이렇게 쓰고 싶다"는 사람의 생각만은 비교적 안 변한다. Scenario-First 는 그 한 줄을 정답(SoT)으로 두고 거기서 **거꾸로** 내려간다 — 시나리오에서 spec 을, spec 에서 코드를.

통과 여부는 사람의 "맞나?" 가 아니라 자동 게이트가 판정하고, 루프는 **네가 실제로 써봐야** 닫힌다 — "검증된 듯 쓸모없는" 결과물이 쌓이는 걸 막으려고.
