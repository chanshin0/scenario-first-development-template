...
개발의 장벽이 무너졌고, 이제 누구나 바이브 코딩을 할 수 있는 시대가 됐습니다. 그렇다면 **보통 사람**(*컴퓨팅 사고를 배운 적 없는*)에게 "개발"이란 뭘까요? 거창한 정의는 필요 없습니다. **"내가 쓰고 싶은 방식대로 동작하게 해줘."** 그게 그들이 바라는 개발입니다.

이 시스템은 그 니즈를 그대로 반영합니다. 대부분의 사람에게는 구체적인 명세를 쓰는 것보다, 머릿속의 사용 장면 하나를 떠올리는 편이 훨씬 쉽습니다.

**구체적으로 어떻게 쓰고 싶은지, 사용 장면 하나만 던지세요.** 구조화된 파이프라인이 그 시나리오를 받아 실제로 만들어 냅니다. 안에서 무슨 일이 벌어지는지는 **몰라도 됩니다.** 같은 시나리오를 직접 해봤을 때 결과물이 의도한 대로 동작하는지, 그것만 확인하세요.

<details>
<summary>그래도 안에서 무슨 일이 일어나는지 궁금하다면</summary>

1. 그 시나리오를 홀로 설 수 있는 **최소 MVP 크기**로 펼칩니다.
2. 그걸 구체적인 **명세로 확정**하고 약속합니다.
3. 당신의 **시나리오가 통과할 때까지** 코드를 짭니다.
4. 자, 이제 **눌러보세요** — 당신이 원하던 게 맞나요?

</details>

이 레포는 **GitHub template** 입니다. "Use this template" 으로 clone 하면 7 스킬 + 메타 에이전트가 이미 들어있습니다 — 외부 install·마켓플레이스·trust dialog 없음.
...
이게 전부예요. `throw`(1) → `expand`(2) → `spec`(3) 까지 그대로 흘러가면 됩니다.
**E2E 프레임워크는 처음 `/scenario-first-goal`(4) 돌릴 때 한 번만 물어봐요** — 시작 시점에 정할 필요 없습니다.

> clone 직후에는 SFD 자신의 흔적(미치환 placeholder, 이 README)이 조금 남아 있어요. 거슬리면 `/scenario-first-init` 한 번으로 치울 수 있습니다 — 선택이고 멱등입니다. (자세히는 아래 [clone 정리](#clone-정리) 참고.)

---

> **여기까지가 사용자가 알아야 할 전부예요.** 아래부터는 직접 만들거나, 안에서 무슨 일이 벌어지는지 궁금한 사람을 위한 내부 이야기입니다.
...
└────────────┘──────────► REVIEW.md
     ...
     │       ├─ (post-MVP, depth) 통과 NNN 정밀화 → deepen: expanded 에 example 누적 → goal → review 재통과
     │       └─ (post-MVP, cosmetic) 외관만 다듬기 → tweak: 기존 풀 green 유지 + 시각 evidence
     └─ fail → (1) throw 갱신 / (2) spec 재실행 / (3) goal 재개
```

**단방향. 역동기 없음. 한 번에 하나 (throw-cycle NNN 또는 deepen-NNN).** (tweak 은 cycle 아님 — lock 미점유.)
**3축**: breadth=throw(새 backbone) / depth=deepen(있는 걸 정밀하게) / cosmetic=tweak(외관, 행동 델타 없음). 둘 다 post-MVP. 구별: 새 GWT(Then) 쓸 수 있으면 deepen, 없으면 tweak.

---

## 설계 철학 — 4 기둥

### 1. 정답은 코드도 spec도 아니라 "이렇게 쓰고 싶다" 이다

코드는 시간이 지나면 다시 쓰이고, spec 은 낡는다. 하지만 *"이렇게 쓰고 싶다"* 는 마음은 비교적 오래 남는다. 그래서 SoT(Source of Truth)를 그 **사용 시나리오**에 두고, `scenarios/` 로 꺼내 둔다.

왜 User Story 가 아니라 **Job Story** 인가:

| User Story 의 약점 | Job Story 가 메우는 법 |
|---|---|
| 페르소나 가정("개발자가…") — 그게 내가 아니면 정답이 틀어짐 | 페르소나를 빼고 **상황**만 남김 |
| 사후 합리화 동기("효율을 위해") — 쓰는 순간의 진짜 동기 누락 | 동기를 그 순간에 고정 |
...
Job Story 에서 게이트까지 가는 *방식*이 매번 다르면 결과 품질도 매번 달라진다. 그래서 같은 4 도구를 같은 순서로 돌린다:
...
절차를 고정하면 결과의 흔들림이 줄어든다.

### 3. 게이트가 정답이다 — 사람이 "맞나?" 묻지 않는다

GWT 시나리오 자체가 정답을 정의한다.
...
- 수동 확인은 게이트에 넣지 않는다 — 그건 5번 review 의 일

게이트는 **이번 시나리오 + 누적된 모든 이전 시나리오**다. regression 은 별도 게이트가 아니라 이 누적 풀에 흡수된다.

thrashing 은 **진전 신호 3종**으로 막는다 — 비용($)이 아니라 진전 자체를 측정한다:
`STUCK_RETRIES`(같은 실패 hash 반복) · `NO_PROGRESS`(PASS 수 동결) · `MAX_ITERATIONS`(반복 상한).

### 4. 직접 안 쓰면 시스템이 죽는다

자동 게이트를 다 통과해도 *직접 써보지 않으면* 가짜 정답이다 — "검증된 듯하지만 쓸모없는 결과물"이 쌓이는 **주기적 자동 MVP 함정**.

5번 review 는 이 함정의 마지막 차단막이다:

- 체크리스트 = Job Story 의 `so I can…` + walking skeleton 한 줄 (직접 쓴 것)
- **Claude 는 시뮬레이션 금지** — 직접 써볼 때까지 멈춤
- 실패하면 5 Whys 로 (1) 시나리오 / (2) spec / (3) 구현 중 하나로 라우팅
- "그냥 둬" 라는 4번째 옵션은 없다

이 단계를 건너뛰면 1~4 는 전부 무의미해진다.

---

## 언제 쓰고 · 언제 안 쓰나

**맞는 경우** — 본인이 직접 빌드하고, 직접 쓰고, 직접 평가하는 single-developer mode. "이렇게 쓰고 싶다"가 비교적 안정적인 도구·앱. 이미 핵심 사용 시나리오 하나가 있고 얇은 MVP 를 빠르게 검증하고 싶을 때. cycle 을 반복하며 backbone 슬라이스를 채워가는 진화적 개발.
...
## 운영 layer — 스킬이 모르는 11 룰

5 스킬은 각자 자기 단계만 안다. cycle 전체, 세션 간 상태, 게이트 누적 정책, 하네스 자기 진화는 모른다. 이 메타 layer 는 `.harness/` 가 맡는다.
...
| 10 | **deepen (depth)** — 통과 NNN 해상도 ↑, example **추가만**(단조), `deepen-NNN` lock | AGENTS.md 3.10 |
| 11 | **tweak (cosmetic)** — 행동 델타 없는 외관 변경. 게이트1=기존 풀 green 유지, 게이트2=시각 evidence. 풀 미진입 | AGENTS.md 3.11 |
...
```
my-project/
├── AGENTS.md            # 진입점 — 시작 6단계 + 운영 11룰 + 종료 5단계
...
**책임 분리**: `scenarios/` = SoT(사용자의 사용 의도, 단계별로 추가). `.harness/` = 운영 메타(상태·정책·로그).
...
MIT © 2026 chanshin0 — [LICENSE](LICENSE) 참고.
