...
| 인자 | 동작 |
|---|---|
| `/scenario-first-init` | 빈/기존 cwd 에 `$SCENARIO_FIRST_HOME` 시드 **복사** + placeholder 치환 + E2E 결정 (기존 파일 보존) |
| `/scenario-first-init --from-template` | cwd 가 이미 SFD template clone 일 때 — **복사 없이 정리만**: SFD 메타 제거 + placeholder 치환 + E2E 결정 |
| `/scenario-first-init --force` | 기존 파일 백업 후 덮어쓰기 (`.harness/.backups/init/<ISO8601>/`) — SFD 갱신 받을 때 |
| `/scenario-first-init --dry-run` | 복사·정리·치환 대상만 출력 |
| `/scenario-first-init --check` | 현재 레포의 하네스 상태 진단 (어떤 시드가 빠졌는지) |

자동 트리거 금지. 새 프로젝트 시작 시 본인이 1회 (선택 — 안 돌려도 throw~spec 진행, E2E 는 goal 0번이 처리).

### `--from-template` 모드 (clone 정리 전용)

`Use this template` 로 clone 한 사본은 시드가 **이미 cwd 에 있다**. 복사할 필요 없이, SFD 색만 벗기면 됨:

| 정리 동작 | 대상 |
|---|---|
| placeholder 치환 | `{{PROJECT_NAME}}`→레포명, `{{E2E_FRAMEWORK}}`/`{{E2E_TEST_CMD}}`→E2E 결정값, `{{CREATED_AT}}`→ISO8601 |
| `.env.scenario` 생성 | `.env.scenario.example` → `.env.scenario` (치환값 채움). `.example` 는 보존 |
| E2E 프레임워크 결정 | 아래 2단계 로직 (감지 → 1회 질문 → 선택 설치) |
| SFD 메타 제거 | `README.md` (SFD 방법론 설명서) + `.harness/EVOLUTION/[0-9]*.md` (SFD 자신의 ADR — 새 프로젝트는 001부터 새로 쌓음) |
| git 후속 | `git config --local commit.template .gitmessage` |

**제거 안 함**: `.claude/skills/`·`.claude/agents/`·`.harness/` (EVOLUTION ADR 본문 제외)·`scenarios/`·`tests/e2e/`·`init.sh`·`.gitmessage`·`AGENTS.md`·`CLAUDE.md` — 전부 그대로 쓰는 자산. `scenarios/`·`tests/e2e/` 는 template 에 `.gitkeep` 만 있어 비울 것도 없음.
