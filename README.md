...
이 레포(Scenario-First Development, 줄여서 **SFD**)는 **GitHub template repository** — 새 아이디어 시작 시 "Use this template" 또는 clone 한 사본 위에서 `/scenario-first-init` 호출. 자기완결: 외부 마켓플레이스·플러그인·trust dialog 없이 clone 하나로 6 스킬 + 에이전트 즉시 사용 가능.
...
| `init.sh` · `.gitmessage` · `rules.json` | `README.md` = SFD **방법론 설명서** (네 README 아님) |

> SFD 가 이 템플릿을 진화시킨 기록(하네스 변경)은 **commit message** 에만 있고 `Use this template` 은 git 히스토리를 미상속하므로, 그 기록은 네 clone 에 따라오지 않는다. clone 은 깨끗한 단일 commit 으로 시작.

`/scenario-first-init` (인자 없음) 이 오른쪽 칸을 정리한다:
1. placeholder 치환 (`{{PROJECT_NAME}}`→레포명, E2E 값, 날짜)
2. `.env.scenario` 실파일 생성 + E2E 프레임워크 결정
3. SFD 방법론 `README.md` 제거 (네 프로젝트 README 는 따로)
4. git `commit.template` 등록

**안 돌려도 된다.** throw~spec 은 그대로 동작하고, E2E 는 처음 goal 이 알아서 묻는다. 안 돌리면 placeholder·SFD README 가 남는 **미관 노이즈**만 감수하는 것. 멱등 — 아무 때나 1회 돌리면 됨.
...
| 9 | **하네스 변경 통로** — 하네스(템플릿 레이어) 수정은 `/sfd-architect` 검토 → 적용, 근거는 commit message. 즉흥 변경 금지 | `.claude/agents/sfd-architect.md`, `rules.json: harness_change_via_architect` |
...
│   ├── rules.json                  # 머신 판독 룰 (harness_change_via_architect 포함)
│   ├── templates/
...
| 사용자가 본인이 아닐 때 (외주·납품) | review 5단계의 "본인 사용" 강제 깨짐 |
...
3. (선택) AGENTS.md 훑기 (운영 9룰) · clone 정리하려면 `/scenario-first-init`

하네스(템플릿 레이어) 를 바꾸려 할 때: `/sfd-architect "<변경 의도>"` → 검토 → 적용, 근거는 commit message (룰 3.9).

## 변경 이력

- v3 (현재) — 자기완결 모델. 6 스킬 + sfd-architect 시드 안 박힘. GitHub template repo. sfd-architect 하네스 변경 검토 통로 (룰 3.9). BUDGET → 진전 신호 3종으로 stuck detection 명세 일관. 정본/시드 분리(Plugify) 폐기.
