...
>
> 사람의 이 생각이 **진실**이다.
> 이 한 줄에서 **역으로** 스펙을 세우고 코드를 구현한다.

코드는 다시 쓰이고 spec 은 deprecate 되지만, "이렇게 쓰고 싶다"는 마음은 비교적 안 변한다. 그래서 그 한 줄(시나리오)을 SoT(Source of Truth)로 박고, 거기서 거꾸로 내려간다 — **시나리오 → spec → 코드 → 자동 게이트 → 직접 사용**.

AI 가 코드를 싸게 찍어내는 시대에 진짜 어려운 건 *"맞는 걸 만들었나"* 다. 정답을 사람의 "맞나?" 감각이 아니라 시나리오로 고정하고, 자동으로 측정하고, 마지막엔 **네가 직접 써보게** 강제한다 — "검증된 듯 쓸모없는 결과물"이 쌓이는 함정을 막기 위해.
