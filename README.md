...
> **"코드도 spec도 SoT가 아니다. 네가 결과를 어떻게 쓰는지의 narrative 가 SoT다."**

AI 가 코드를 싸게 찍어내는 시대에 진짜 어려운 건 *"맞는 걸 만들었나"* 다.

Scenario-First 는 그 정답을 사람의 "맞나?" 감각이 아니라 **네가 그걸 어떻게 쓸지(시나리오)** 로 고정하고, 통과 여부를 자동 게이트로 측정한다. 그리고 자동 통과만으로는 가짜이기 때문에 — 마지막에 **네가 직접 써보는 단계**를 강제한다.

혼자 만들고 · 혼자 쓰고 · 혼자 평가하는 **single-developer 진화적 개발**을 위한 닫힌 루프(harness). Job Story → 자동 게이트 → 직접 사용까지 매 cycle **같은 절차**로 흘러서, "이번엔 어떻게 쪼개지?" 하는 즉흥의 비용을 0 으로 만든다.
...
> clone 직후엔 SFD 자신의 흔적(미치환 placeholder, 이 README)이 살짝 묻어있다. 거슬리면 `/scenario-first-init` 한 번으로 치운다 — 선택이고 멱등. (자세히는 아래 [clone 정리](#선택-clone-정리) 참고.)
