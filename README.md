...
코드는 다시 쓰이고 spec 은 deprecate 된다. 비교적 안 변하는 건 **"이런 게 있으면 좋겠다", "이렇게 쓰고 싶다"** 는 사람의 생각뿐이다. Scenario-First 는 그 한 줄을 정답(SoT)으로 두고 거기서 **거꾸로** 내려간다 — 시나리오에서 spec 을, spec 에서 코드를. 통과 여부는 사람의 "맞나?" 가 아니라 자동 게이트가 판정하고, 마지막엔 **네가 직접 써봐야** 끝난다 — "검증된 듯 쓸모없는" 결과물이 쌓이는 걸 막으려고.
